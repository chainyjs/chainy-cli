...
// Chainy
var Chainy = require('chainy-core').subclass().require('set exec each map')
...
Chainy
	...
	.addExtension('injectdefaultpackagedata', 'action', function(packageData, opts){
		packageData.verison = packageData.version || '1.0.0'

		packageData.badges = packageData.badges || {
			"travis": true,
			"npm": true,
			"david": true,
			"daviddev": true
		}
		packageData.directories = packageData.directories || {
			"lib": "./lib",
			"test": "./test"
		}

		if ( packageData.homepage != null ) {
			packageData.bugs = packageData.bugs ||
				{url: packageData.homepage+'/issues'}
			packageData.repository = packageData ||
				{type:'git', url:packageData.homepage+'.git'}
		}

		packageData.maintainers = packageData.maintainers || []
		packageData.contributors = packageData.contributors || []
		packageData.dependencies = packageData.dependencies || {}
		packageData.peerDependencies = packageData.peerDependencies || {}
		packageData.devDependencies = packageData.devDependencies || {
			projectz: '~0.3.14'
		}

		packageData.scripts = packageData.scripts || {
			test: "node ./test/"+(packageData.shortname || packageData.name)+"-test.js"
		}
	})
	.addExtension('arrangepackagekeys', function(packageData){
		return arrangekeys(packageData,
			'title name version description homepage license badges keywords author maintainers contributors bugs repository browsers browser engines dependencies peerDependencies devDependencies directories scripts cakeConfiguration main')
	})
...
Chainy.create()
	...
	.action(function(argv, next){
		...
		if ( !action || action === 'help' || '--help'.indexOf(argv.cooked) !== -1 ) {
			...
			this.create().set(argv.remain.slice(1)).each(function(plugin, next){
				this.create().set(plugin).installplugin().done(next)
			}).done(next)
		}
		else if ( action === 'install-bundle' ) {
			this.create().set(argv.remain.slice(1)).each(function(plugin, next){
				this.create().set(plugin).installbundle().done(next)
			}).done(next)
		}
		else if ( action === 'install' ) {
			this.create().set(argv.remain.slice(1)).each(function(plugin, next){
				this.create().set(plugin).install().done(next)
			}).done(next)
		}
