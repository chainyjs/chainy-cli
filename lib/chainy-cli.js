...
Chainy
	...
	.addExtension('installplugin', 'action', function(name,next){
		...
		return this.create().set('./node_modules/npm/bin/npm install --save chainy chainy-core chainy-plugin-'+name).exec({stdio:'inherit'}).done(next)
	})
	.addExtension('installbundle', 'action', function(name,next){
		console.log('Attempting to install the bundle: '+name)
		return this.create().set('./node_modules/npm/bin/npm install --save chainy chainy-core chainy-bundle-'+name).exec({stdio:'inherit'}).done(next)
	})
